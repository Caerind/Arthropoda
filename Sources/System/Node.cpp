...
{
	if (!Math::equals(mLocalPosition.x, position.x) || !Math::equals(mLocalPosition.y, position.y))
	{
...
void Node::setPosition(const Vector3& position)
{
	...
	{
		if (!Math::equals(mLocalPosition.z, position.z))
		{
			invalidateNodeZ();
		}
...
{
	if (!Math::equals(mLocalRotation, rotation))
	{
...
{
	if (!Math::equals(mLocalPosition.z, z))
	{
		mLocalPosition.z = z;
		invalidateNodeZ();
	}
...
void Node::move(const Vector3& movement)
{
	...
	{
		if (!Math::equals(movement.z, 0.0f))
		{
			invalidateNodeZ();
		}
