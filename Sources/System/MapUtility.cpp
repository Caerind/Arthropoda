...
void MapUtility::getNeighboors(std::vector<Vector2i>& neighbors, const Vector2i& coords, Orientation orientation, bool diag, StaggerIndex staggerIndex, StaggerAxis staggerAxis)
{
	if (orientation == Orientation::Orthogonal)
	{
		neighbors.emplace_back(coords.x, coords.y - 1);
		neighbors.emplace_back(coords.x, coords.y + 1);
		neighbors.emplace_back(coords.x - 1, coords.y);
		neighbors.emplace_back(coords.x + 1, coords.y);
		if (diag)
		{
			neighbors.emplace_back(coords.x + 1, coords.y - 1);
			neighbors.emplace_back(coords.x + 1, coords.y + 1);
			neighbors.emplace_back(coords.x - 1, coords.y + 1);
			neighbors.emplace_back(coords.x - 1, coords.y - 1);
		}
	...
	else if (orientation == Orientation::Staggered)
	{
		...
		if (diag)
		{
			neighbors.emplace_back(coords.x, coords.y - 2);
			neighbors.emplace_back(coords.x + 1, coords.y);
			neighbors.emplace_back(coords.x, coords.y + 2);
			neighbors.emplace_back(coords.x - 1, coords.y);
		}
...
Vector2 MapUtility::coordsToWorld(const Vector2i& coords, Orientation orientation, const Vector2i& tileSize, StaggerIndex staggerIndex, StaggerAxis staggerAxis, U32 hexSide)
{
	...
	else if (orientation == Orientation::Staggered)
	{
		if (staggerAxis == StaggerAxis::Y)
		{
			...
			p.y = (F32)(coords.y * tileSize.y) * 0.5f;
			return p;
		}
		else
		{
			p.x = (F32)(coords.x * tileSize.x) * 0.5f;
