  ...
  if (mparams.magic == 0) {
    ...
    */
    if ((sizeof(size_t) != sizeof(char*)) ||
        ...
        (sizeof(int) < 4)  ||
        (MALLOC_ALIGNMENT < (size_t)8U) || //-V501
      ...
      if (!is_extern_segment(sp)) {
        if (is_mmapped_segment(sp)) {
          if (HAVE_MMAP &&
            ...
            if ((CALL_MREMAP(sp->base, sp->size, newsize, 0) != MFAIL) || //-V501
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) { //-V658
...
size_t mspace_usable_size(void* mem) { //-V524
  if (mem != 0) {
    mchunkptr p = mem2chunk(mem);
    if (cinuse(p))
      return chunksize(p) - overhead_for(p);
